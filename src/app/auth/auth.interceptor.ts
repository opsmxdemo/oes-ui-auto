   ...
   intercept(req: HttpRequest<any>, next: HttpHandler) {
       return this.store.select('auth').pipe(
        ...
        switchMap((authState: fromAuth.State) => {
          ...
          if(requrl.includes('/auth/user') || requrl.includes('dashboard') || requrl.includes('autopilot') || requrl.includes('oes') || requrl.includes('dashboardservice') || requrl.includes('auth/logout') || requrl.includes('app-config.json')){

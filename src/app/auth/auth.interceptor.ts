   ...
   intercept(req: HttpRequest<any>, next: HttpHandler) {
       return this.store.select('auth').pipe(
        ...
        switchMap((authState: fromAuth.State) => {
          ...
          if(requrl.includes('/auth/user') || requrl.includes('platformservice') || requrl.includes('autopilot') || requrl.includes('oes') || requrl.includes('auth/logout') || requrl.includes('app-config.json')){
